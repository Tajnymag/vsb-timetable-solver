...
export function slotToTime(fromSlot: number, toSlot: number): [string, string] {
...
export function overlaps(eventA: TimetableEvent, eventB: TimetableEvent) {
...
(async () => {
	...
	const timetableContainer = await page.$<HTMLTableElement>('.dataTable.evenOdd.specialGray');
	...
	const timetableOpenBtns = await page.$$<HTMLAnchorElement>('.wpthemeControlBody > div a[title="Zvolit"]:not([href="#"])');
	...
	for (const timetableOpenBtn of timetableOpenBtns) {
		...
		const schedTable = await page.$<HTMLTableElement>('.wpthemeControlBody .schedTable');
		...
		const eventEls = await schedTable.$$<HTMLTableElement>('.actTable');
		...
		for (const eventEl of eventEls) {
			...
			const day = await eventEl.evaluate(el => (el?.parentElement?.parentElement as HTMLTableRowElement)?.rowIndex / 2 - 1);

			const beginSlot = await eventEl.evaluate(el => {
				const prevAll = (element: HTMLElement | null) => {
					let result = [];
					// @ts-ignore
				...
				let offset = prevAll(el?.parentElement).reduce((acc, curr) => {
					let colSpan = (curr as HTMLTableCellElement).colSpan || 1;
					return acc + colSpan;
				}, 0);
			...
			const endSlot = await eventEl.evaluate((el, beginSlot) => {
				const columnEl = el.parentElement as HTMLTableCellElement;
	...
	const eventsSortedMap = new Map<string, { lectures: TimetableEvent[], practices: TimetableEvent[] }>();
	...
	for (const possibility of cartesianProduct) {
		const combinations = new Combination(possibility, 2);
	...
	if (argv.output) {
		writeFileSync('possibilities.json', JSON.stringify(cleanedUpPossibilities));
	}
	if (argv.stdout) {
